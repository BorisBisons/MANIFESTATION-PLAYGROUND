...
**Commands:** `add "text"` · `list` · `edit <id> "new text"` · `pause <id>` / `resume <id>` · `remove <id>` (soft delete) · `times 08:00 13:00 21:00` (rewrites the plist and reloads launchd) · `send-now` (test send, logged as slot `manual`) · `stats` · `channel ntfy --topic X` (fallback channel, iMessage is the default).
