...
- [ ] Reconcile the audit's remaining findings + fix plan when it completes; then a second review workflow over the final diff
- [ ] Tests: 36 pass on Linux with the sender mocked (14 new: early-window dedupe, failed-then-retried, retry-before-fail, catch-up pause/resume, schedule change, once-a-day shuffle, fresh install, cross-day occurrences, pre-`slot_at` rows, DB migration, wakes, status, power log, shuffle plist)
